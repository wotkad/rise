...
* Скрипт `preloader.js` отображает анимацию загрузки пока страница загружается.

## 6.4 Offline Detector

* Отслеживает `navigator.onLine`, показывает баннер “Вы офлайн” если пропала сеть и кеширует последние данные.

## 6.5 Автоматическая высота textarea

`textarea-autoheight.js` Этот скрипт отвечает за автоматическое изменение высоты `<textarea>` под содержимое — то есть делает текстовые поля “растягивающимися” по мере ввода текста.
...
* Скрипт для `<svg><use>` (Этот скрипт нужен для автоматического переноса размеров и viewBox из <symbol> в сам <svg>, который использует этот символ через тег <use>.)
...
* Скрипт получения и установки текущего года.
