...
При добавлении, компонент создаётся в проекте в папках `/components/`, для PUG, SCSS, JS и `/assets/`, для изображений. Все прочие компоненты находятся в `/custom-components/`.
