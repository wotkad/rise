...
Шаблон значительно упрощает такие вещи как: сборка проекта, поиск необходимых библиотек для старта проекта, генерацию карты сайта, настройку базового SEO, настройку архитектуры проекта, оптимизацию и сбор аналитики, а также предлагает создание контента в формате `.md`.
...
- **Node.js**: [Node.js](https://nodejs.org/en/download)
  - Для запуска шаблона на локальном компьютере необходим Node.js, его можно скачать по ссылке выше (версия 22.21.0 или выше).
- **Yarn**: [Yarn](https://yarnpkg.com/lang/en/docs/install)
  - Также необходимо выполнить в консоли команду `brew install yarn` для установки Yarn или следовать специальному руководству по ссылке выше.

### 👨‍💻 Запуск шаблона

1. Для начала работы требуется создать новый репозиторий на основе [шаблона](https://github.com/new?template_name=rise&template_owner=wotkad).
2. Открыть директорию `название проекта` в редакторе кода.
3. Выполнить команду `yarn` для установки зависимострей проекта.
4. Запустить процесс разработки в проекте можно командой `yarn dev`.
5. Приступить к разработке.

### 📄 Добавление страниц

- Для создания новой страницы, необходимо создать её в директории `/views/pages`, после нужно перезапустить проект, тогда новая страница станет доступна.
- Для создания любого раздела, например, `/products/product`, где `/products` - родительский раздел, а `/product` - дочерняя, нужно в файле `webpack.config.js`, в объект `plugins`, добавить код `...pager.pages(MODE, "products")`.
...
- `plugins`: Вспомогательные скрипты.
  ...
  - `404.js`: Скрипт для создания файла `/404.html` в директории - `/build`.
    ...
    - `js`: Директория для файлов `.js`. (Можно создать любую структуру).
      ...
      - `base`: Основные базовые скрипты проекта.
        ...
        - `common`: Директория с базовыми функциями.
          ...
          - `service-worker.js`:
          - `set-header-bg.js`: Скрипт для установки класса для `<header>`.
          ...
          - `textarea-autoheight`: Скрипт для автоматической установки высоты `<textarea>`.
