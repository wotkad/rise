...
9. Тестирование возможно через `http-server` установив его командой `yarn global add http-server`. Выполнив команду `npx http-server -p 9090 ./dist`, по URL-адресу `http://localhost:9090` можно увидеть сборку в действии.
10. Broken Link Checker - Скрипт для проверки работоспособности ссылок на всём сайте (выполните команду `node blc` при запущенном проекте.)
...
- `src`: Весь исходный код содержится здесь. Ресурсы, `.js`, `.scss`, `.pug` и т. д. **Любые изменения, которые вы хотите внести на веб-сайт, должны быть сделаны здесь.**
  ...
  - `scripts`: Вспомогательные скрипты.
    ...
    - `createSitemap.js`: Скрипт для создания карты сайта в директорию `/src`.
