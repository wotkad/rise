...
- При необходимости создания архива, необходимо запустить скрипт `yarn archive-paths` для обновления путей до всех ресурсов (папка `assets`), чтобы `.html` файлы корректно отображались.
...
- `node_scripts`: Вспомогательные скрипты.
  - `archive-paths.js`: Скрипт для перезаписывания ссылок в проекте и их актуализация если проект необходимо просмотреть без запуски сервера.
