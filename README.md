...
Для добавления в проект выполните команду `yarn constructor my-component`. Компонент будет добавлен в проект, если он существует. Список и визуальное представление всех доступных компонентов можно посмотреть тут (`/reports/constructor`).

После генерации выбранный компонент останется только подключить в нужное место в `.pug` файлах.

* В случае если необходимо обновить компонент воспользуйтесь командой `yarn constructor my-component --rewrite`, где флаг `--rewrite` указывает на то, чтобы компонент был обновлён до его версии исходной версии.
