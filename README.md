...
Для добавления в проект выполните команду `yarn constructor my-component`. Компонент будет добавлен в проект, если он существует в библиотеке компонентов. Список и визуальное представление всех доступных компонентов можно посмотреть тут (`/reports/constructor/report.html`).

После генерации выбранный компонент останется только подключить в нужное место в `.pug` файлах.

* В случае если необходимо обновить компонент воспользуйтесь командой `yarn constructor my-component --rewrite`, где флаг `--rewrite` указывает на то, чтобы компонент был обновлён до его исходной версии.

* В случае если необходимо удалить компонент воспользуйтесь командой `yarn constructor my-component --remove`, где флаг `--remove` указывает на то, что компонент должен быть удалён.
...
/js/                         # глобальный JS
  └── <component-name>.js    # логика, используемая в компоненте (должен быть одноимённым с названием компонента, содержит всю доступную логику для всех версий компонента)
```

* Компоненты могут быть разных версий (v1/v2...).
* Компоненты могут содержать другие компоненты.

При создании компоненты создаётся в проекте в папках `/components/`, для PUG, SCSS, JS и `/assets/`, для изображений. Все прочие компоненты могут находиться в `/custom-components/`.
