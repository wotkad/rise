...
**Rise** - это быстрый, лёгкий в использовании и настройке шаблон, с помощью которого можно реализовать проект любой сложности - от простого лендинга до высоконагруженного сервиса.

Шаблон значительно упрощает такие вещи, как: сборка проекта, поиск необходимых библиотек для старта, генерация карты сайта, настройка базового SEO, организация архитектуры, оптимизация и сбор аналитики, а также создание контента в формате `.md`.

**Шаблон включает в себя:**
- Конструктор (библиотека готовых секций)
- Шаблонизацию в `.pug` и `.scss`
- Простое подключение библиотек `css` и `js`
- Удобную архитектуру
- Генерацию статей в формате `.md`
- Легкую настройку базового SEO
- Тестирование
- Сбор аналитики
- и многое другое... 
...
- **Node.js**: [Node.js](https://nodejs.org/en/download)
  - Для запуска шаблона на локальном компьютере необходим Node.js (версия 22.21.0 или выше).
- **Yarn**: [Yarn](https://yarnpkg.com/lang/en/docs/install)
  - Для установки Yarn выполните команду `brew install yarn` или следуйте официальному руководству по ссылке выше.

### 👨‍💻 Запуск шаблона

1. Создайте новый репозиторий на основе [шаблона](https://github.com/new?template_name=rise&template_owner=wotkad).
2. Откройте директорию проекта в редакторе кода.
3. Выполните команду `yarn` для установки зависимостей.
4. Запустите процесс разработки командой `yarn dev`.
...
- Карта сайта генерируется во время сборки и добавляется в корень проекта `/build/sitemap.xml`.

### 📦 Сборка проекта

- Для сборки проекта выполните команду `yarn build`. Готовая версия будет размещена в директории `/build`.

### 🔬 Тестирование

- В шаблоне предусмотрено два глобальных линтера: для `.pug` и для `.scss` файлов. Запуск: `yarn lint:all` (или по отдельности: `yarn lint:pug`, `yarn lint:scss`). При сборке линтер запускается автоматически.
- Для тестирования проекта можно использовать `http-server`. После сборки `yarn build` выполните `npx http-server -p 9090 ./build`. Готовый проект будет доступен по адресу `http://localhost:9090`.
- Проверить доступность всех ссылок можно при помощи пакета `linkinator`. Запустить скрипт - `yarn linkinator`.
...
- **Barba**: [https://barba.js.org](https://barba.js.org)
...
- **SmoothScroll**: [Smooth-Scroll.js](https://github.com/cferdinandi/smooth-scroll)
  - Легкий скрипт для анимации прокрутки к якорным ссылкам.

### 🖋 Шрифты

- **Inter**: [Inter](https://fonts.google.com/specimen/Inter)
  - Подключать/редактировать/удалять шрифты можно в файле `src/assets/styles/base/fonts.scss`.
  - Основной шрифт указан в файле `src/assets/styles/base/global.scss`.

### 🖼 Иконки

- **FontAwesome**: [https://fontawesome.com](https://fontawesome.com)
  - Иконочный шрифт. Использовать его можно так: `i.fa.fa-angle-right`, где первый класс `.fa` отвечает за стиль иконки. Начертание может быть обычным, жирным, тонким или являться брендовым стилем, что позволяет использовать, например, иконку Twitter `i.fa-brands.fa-twitter`. Список всех начертаний: `fa-brands`, `fa-solid`, `fa-regular`, `fa-light`, `fa-thin`, `fa-duotone`, `fa-brands`.