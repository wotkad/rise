...
🔧 Конструктор компонентов - автоматическая генераций UI-блоков с шаблоном, стилем и логикой (yarn constructor) сразу в проект.
...
reports/       # отчеты (Lighthouse, Accessibility, Размер файлов)
```
...
* **SPA/Multi-page:** Rise поддерживает плавные переходы между страницами и многостраничные сайты, благодаря `Barba.js`.
...
* Компоненты могут использовать вложенные компоненты.
...
## 4.7 Иконочный шрифт Font Awesome 6.0.0
...
* Автоисправление через Prettier и Stylelint
...
Smart Lazy Loader кеширует изображения и подгружает placeholder с dominant color.
...
#### 5. Lighthouse Audit — анализ производительности

```bash
yarn reports:lighthouse
```
Запускает Lighthouse CLI в headless-режиме (без браузера).
...
- `reports`: Директория для отчётов.
  ...
  - `lighthouse`: Отчёт lighthouse (работает только в dev-режиме).
